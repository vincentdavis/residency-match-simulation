...
class StatsAndplots(object):
    ...
    def plots(self):
        ...
        show()
        print('Plot of')
        pyplot.plot([x.quality for x in self.all_institutions if len(x.matched_to) == x.openings], [np.average([y.quality for y in x.matched_to]) for x in self.all_institutions if len(x.matched_to) == x.openings])
        show()
        pyplot.plot([x.quality for x in self.all_institutions if len(x.matched_to) == x.openings], [np.min([y.quality for y in x.matched_to]) for x in self.all_institutions if len(x.matched_to) == x.openings])
        show()
        pyplot.plot([x.quality for x in self.all_institutions if len(x.matched_to) == x.openings], [np.max([y.quality for y in x.matched_to]) for x in self.all_institutions if len(x.matched_to) == x.openings])
        show()
